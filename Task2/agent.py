...
##############################################################################################################################
# TO DO : Check the gs://{project_id}-labconfig-bucket/ bucket to validate the name of the products briefing file 
##############################################################################################################################
...
##############################################################################################################################
# TODO: Pass the read_gcs_file function to the agent as FunctionTool so it can access product brief data from Cloud Storage.
##############################################################################################################################

# CORRECTION 1 : On transforme la fonction Python en "Outil" pour l'agent
...
""",
    # CORRECTION 2 : On donne l'outil créé ci-dessus à l'agent
    tools=[gcs_reader_tool],
    output_key="key_features"
)

##################################################################################################
# TODO: Integrate the built in tool so the agent can perform web searches through Google Search.
##################################################################################################
...
""",
    # CORRECTION 3 : On ajoute l'outil google_search (déjà importé en haut du fichier)
    tools=[google_search],

...
##############################################################################################################################
# TODO : Pass the convert_currency function to the agent as FunctionTool to convert currency using external API.
# ADK Function Tools : https://google.github.io/adk-docs/tools-custom/function-tools/
##############################################################################################################################

# CORRECTION 1 : Création de l'outil à partir de la fonction
...
,output_key="financial_analyst_response",
    # CORRECTION 2 : Ajout de l'outil à l'agent
    tools=[currency_converter_tool]
)


# Do not change — used for activity tracking.
# CORRECTION : Sécurisation du log pour éviter l'erreur 403 en local
try:
    logger.log_text(
        f"tools: {financial_analyst_agent.tools}"
    )
except Exception as e:
    print(f"Warning: Logging failed (ignored for local testing): {e}")
